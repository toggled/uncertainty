...
#create directories
for dir_name in ["data/ER", "data/BA"]:
...
def generate_queries_skewed(g, filename):
    ...
    for hops in sorted(queries):
        if(len(queries[hops])):
            q = open(filename+"_"+str(hops)+".queries", "w")
...
def BA(graph_sizes, density):
    ...
    f = open("data/BA/BA_{}_{}.graph".format(n, m), "w")
    f.write("{} {}\n".format(n, m))
    for u, v in g.edges:
        l = random.randint(1, MAX_EDGE_LENGTH)
        p = random.random()
    ...
    generate_queries_skewed(g, "data/BA/BA_{}_{}".format(n, m))
    # generate_queries_single(g, "data/BA/BA_{}_{}_".format(n, m))
